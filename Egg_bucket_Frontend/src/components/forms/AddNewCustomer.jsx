...
const AddNewCustomer = () => {
  ...
  useEffect(() => {
    ...
    // Fetch all customers
    fetch('http://localhost:3577/customers/egg-bucket-b2b/getAllCustomer')
      .then(response => response.json())
      .then(data => {
        if (data.length > 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Submit form data
      const response = await fetch('http://localhost:3577/customers/egg-bucket-b2b/create-customer', {
        method: 'POST',
        body: formDataToSend,
      });
